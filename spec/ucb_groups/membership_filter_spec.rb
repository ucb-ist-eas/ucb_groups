...
describe "MembershipFilter" do
  ...
  it "creates groups filter" do
    ...
    filter_str = "(&(objectclass=person)" +
                    "(berkeleyeduismemberof=cn=edu:berkeley:app:calmessages:deans,ou=campus groups,dc=berkeley,dc=edu))"
    filter.should eql(filter_str)

    filter = UcbGroups::MembershipFilter.new(:calmessages, [:deans, :instructors], []).to_s
    filter_str = "(&(objectclass=person)" +
                    "(|(berkeleyeduismemberof=cn=edu:berkeley:app:calmessages:deans,ou=campus groups,dc=berkeley,dc=edu)" +
                      "(berkeleyeduismemberof=cn=edu:berkeley:app:calmessages:instructors,ou=campus groups,dc=berkeley,dc=edu)))"
    filter.should eql(filter_str)
  end
  ...
  it "creates groups and orgs filter" do
    ...
    filter_str = "(&(objectclass=person)" +
                   "(&(|(berkeleyeduismemberof=cn=edu:berkeley:app:calmessages:deans,ou=campus groups,dc=berkeley,dc=edu)" +
                       "(berkeleyeduismemberof=cn=edu:berkeley:app:calmessages:instructors,ou=campus groups,dc=berkeley,dc=edu))" +
                     "(|(departmentnumber=*JKASD*)(departmentnumber=*JJCNS*))))"
    filter.should eql(filter_str)
  end
end
